...
class State():
    ...
    def step(self, act, inner_state):
        ...
        if self.image.shape[0] > 1:
            ...
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range, rg, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+1, rb, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+2, gb, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+3, r_plus, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+4, g_plus, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+5, b_plus, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+6, r_min, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+7, g_min, self.image)
            self.image = np.where(act[:,np.newaxis,:,:]==self.move_range+8, b_min, self.image)
        else:
            ...
            self.image = np.where(act[:,:,:,:]==self.move_range, rg, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+1, rb, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+2, gb, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+3, r_plus, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+4, g_plus, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+5, b_plus, self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+6, r_min,  self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+7, g_min,  self.image)
            self.image = np.where(act[:,:,:,:]==self.move_range+8, b_min,  self.image)
